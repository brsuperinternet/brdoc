...
import { isErrorCell, makeErrorCell } from "./error";
...
export function evaluate(
  ...
  switch (ast.t) {
    ...
    case "null":
      return null;
    case "prop":
      return evalProp(ast.id, row, ctx, depth);
    case "op":
      return evalOp(ast.op, ast.args, row, ctx, depth);
    case "if": {
    ...
    case "call": {
      ...
      try {
        ...
        return makeErrorCell("TYPE_MISMATCH", (e as Error).message);
      }
    }
  }
}
...
function evalProp(
  ...
  if (!prop) {
    return makeErrorCell("MISSING_PROP", `missing property ${id}`);
  }
  ...
  if (ctx.depth >= ctx.maxDepth) {
    return makeErrorCell("DEPTH_EXCEEDED", `max depth ${ctx.maxDepth}`);
  }
