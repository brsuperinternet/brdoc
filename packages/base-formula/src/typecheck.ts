...
function infer(
  ...
  switch (ast.t) {
    ...
    case "null":
      return "null";
    case "prop":
      return propertyTypes.get(ast.id) ?? "null";
    case "op": {
    ...
    case "call": {
      ...
      const argTypes = ast.args.map((a) => infer(a, propertyTypes, registry));
      if (
        argTypes.length < fn.arity.min ||
        (fn.arity.max != null && argTypes.length > fn.arity.max)
      ) {
        throw new FormulaParseError([
          {
            code: "ARITY_MISMATCH",
...
function typeErr(message: string): FormulaParseError {
  return new FormulaParseError([
    { code: "TYPE_MISMATCH", message, span: { end: 0, start: 0 } },
  ]);
}
