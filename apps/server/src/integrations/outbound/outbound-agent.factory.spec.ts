...
describe("OutboundAgentFactory", () => {
  it("validates the URL through the guard and returns a releasable undici Agent", async () => {
    ...
    const lease = await factory.lease("https://siem.example.com/ingest", {
      caCert: undefined,
  ...
  it("propagates guard rejections", async () => {
    const validate = jest
      .fn()
      .mockRejectedValue(
        new OutboundUrlError("Destination URL must use https")
      );
    const factory = new OutboundAgentFactory({ validate } as any);

    await expect(factory.lease("http://siem.example.com")).rejects.toThrow(
      OutboundUrlError
    );
  });
});
