...
export function isAlwaysBlockedAddress(ip: string): boolean {
  if (isIPv4(ip)) {
    return ALWAYS_BLOCKED.check(ip, "ipv4");
  }
  if (isIPv6(ip)) {
    return ALWAYS_BLOCKED.check(ip, "ipv6");
  }
  return true;
}
...
export function isHardBlockedAddress(ip: string): boolean {
  if (isIPv4(ip)) {
    return HARD_BLOCKED.check(ip, "ipv4");
  }
  if (isIPv6(ip)) {
    return HARD_BLOCKED.check(ip, "ipv6");
  }
  return true;
}
...
export function isPrivateNetworkAddress(ip: string): boolean {
  if (isIPv4(ip)) {
    return PRIVATE_NETWORKS.check(ip, "ipv4");
  }
  if (isIPv6(ip)) {
    return PRIVATE_NETWORKS.check(ip, "ipv6");
  }
  return true;
}
...
function findRefusal(
  ...
  for (const { address } of resolved) {
    ...
    if (isHardBlockedAddress(address)) {
      return { address, kind: "hard-blocked" };
    }
    if (policyNamesAddress(policy, address, port)) {
      continue;
    }
