...
export async function up(db: Kysely<any>): Promise<void> {
  await db.schema
    ...
    .execute();
  await db.schema
    .createIndex("oauth_clients_workspace_id_idx")
    .on("oauth_clients")
    .column("workspace_id")
    .execute();
  ...
  await db.schema
    ...
    .execute();
  await db.schema
    .createIndex("oauth_authorization_codes_consumed_at_idx")
    .on("oauth_authorization_codes")
    .column("consumed_at")
    .execute();
  await db.schema
    .createIndex("oauth_authorization_codes_client_id_idx")
    .on("oauth_authorization_codes")
    .column("client_id")
    .execute();
  await db.schema
    .createIndex("oauth_authorization_codes_user_id_idx")
    .on("oauth_authorization_codes")
    .column("user_id")
    .execute();
  await db.schema
    .createIndex("oauth_authorization_codes_workspace_id_idx")
    .on("oauth_authorization_codes")
    .column("workspace_id")
    .execute();
  ...
  await db.schema
    .createIndex("oauth_grants_client_id_idx")
    .on("oauth_grants")
    .column("client_id")
    .execute();
  ...
  await db.schema
    ...
    .execute();
  await db.schema
    .createIndex("oauth_tokens_access_token_jti_idx")
    .on("oauth_tokens")
    .columns(["workspace_id", "access_token_jti"])
    .execute();
  // One index per branch of the cleanup sweep's OR so it can bitmap-or them.
  await db.schema
    .createIndex("oauth_tokens_access_expires_at_idx")
    .on("oauth_tokens")
    .column("access_expires_at")
    .execute();
  await db.schema
    .createIndex("oauth_tokens_refresh_expires_at_idx")
    .on("oauth_tokens")
    .column("refresh_expires_at")
    .execute();
  await db.schema
    .createIndex("oauth_tokens_revoked_at_idx")
    .on("oauth_tokens")
    .column("revoked_at")
    .execute();
}
